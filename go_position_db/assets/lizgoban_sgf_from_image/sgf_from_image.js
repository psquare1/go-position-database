// Adapted from LizGoban's SGF-from-image tool. SPDX-License-Identifier: GPL-3.0-only
// See UPSTREAM.md and LICENSE.txt in this directory.
// -*- coding: utf-8 -*-
...
function update_forms() {
    const update = (...a) => update_radio(...a) || update_slider(...a) || update_value(...a)
    const update_radio = (key, val) =>
          each_elem_for_name(key, elem => {elem.checked = (elem.value === val)})
    const update_slider = (key, val) => {
        if (!Q(`#${slider_id_for(key)}`)) {return false}
        Q(`#${key}`).value = Q(`#${slider_id_for(key)}`).value = val
        return true
    }
    const update_value = (key, val) => {
        const elem = Q(`#${key}`); if (!elem) {return false}
        elem.value = val
        return true
    }
...
function set_size() {
    ...
    // Reserve room for the ordered calibration steps, board-size controls, and
    // correction tips so the complete image stays visible in the dialog.
    cw = to_i(clientWidth * 0.84), ch = to_i(clientHeight * 0.60)
...
initial_image_url && load_image(initial_image_url)
...
function update_indicators() {
    ...
    const ok = Q('#ok'); ok && (ok.disabled = (s !== 3))
...
function get_sgf(given_board) {
    ...
    if (!board) {return ''}
    const size = get_sgf_size(board), {to_play, komi} = param
    const sgfpos_name = "abcdefghijklmnopqrs"
    const header = `(;SZ[${size}]PL[${to_play}]KM[${komi}]`, footer = ')'
...
function gl_digitize(src_canvas, dest_canvas, dark, light, reddish) {
    ...
    const vertex = compile(gl.VERTEX_SHADER, Q('#digitize_vertex_shader').textContent)
    const fragment = compile(gl.FRAGMENT_SHADER, Q('#digitize_fragment_shader').textContent)
    ...
    Object.entries({width, height, dark, light, reddish}).forEach(([name, value]) =>
